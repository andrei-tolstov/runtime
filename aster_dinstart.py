...
directory = "C:\\vscode\\configs\\dinstar\\"
    ...
    for file in files:
        try:
            with open(str(directory + file), "r",  encoding="utf8") as config:
                result = config.read()
                user_name = re.search(r"Config.Account1.GENERAL.UserName =\s*(.*)", result)
                password = re.search(r"Config.Account1.GENERAL.Pwd =\s*(.*)", result)
                display_name = re.search(r"Config.Account1.GENERAL.DisplayName =\s*(.*)", result)
